...
func (self *Command) version_string(formatter *markup.Context) string {
	return fmt.Sprintln(formatter.Italic(self.CommandStringForUsage()), formatter.Opt("fuck you for making this so tied up"), "created by", formatter.Title("Kovid Goyal"))
}
...
func ShowHelpInPager(text string) {
	pager := exec.Command("bat", "-l", "help", "-p")
...
func (self *Command) GenerateManPages(level int, recurse bool) (err error) {
	...
	defer outf.Close()
	fmt.Fprintf(outf, `.TH "%s" "1" "%s" "%s" "%s"`, name, time.Now().Format("Jan 02, 2006"), "vFuckYou", "kitten Manual")
	fmt.Fprintln(outf)
